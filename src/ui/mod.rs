...
let (_, filter_mode, _, _, _, _) = app.filter_info();
    ...
    if help_active {
        ...
        let total_input = (search_active as u16) * input_height + (edit_active as u16) * input_height + (command_active as u16) * input_height + (replace_active as u16) * input_height + (filter_mode as u16) * filter_height;
    ...
    if palette_active {
        ...
        render_palette(frame, palette_area, palette_query, palette_cursor, palette_commands);
    }
}
...
fn render_table(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let visible_rows = available_height.min(row_count.saturating_sub(scroll_offset)).max(1);
    ...
    let col_widths: Vec<Constraint> = std::iter::once(Constraint::Length(row_index_width))
        .chain((0..column_count).map(|i| {
            ...
            Constraint::Length(max_width.max(headers.get(i).map(|h| h.len()).unwrap_or(0)).min(30) as u16)
        }))
        .collect();
    ...
    let rows: Vec<ratatui::widgets::Row> = (scroll_offset..scroll_offset + visible_rows)
        .filter_map(|row_idx| {
            ...
            let is_duplicate = if highlight_duplicates {
                duplicate_rows.iter().any(|(_, rows)| rows.contains(&row_idx))
            } else {
                false
            };
    ...
    let header_cells: Vec<ratatui::widgets::Cell> = std::iter::once(
        ratatui::widgets::Cell::from("  #  ").style(Style::new().bold())
    )
...
fn render_status_bar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header = data.headers().get(selected_col).map(|h| h.as_str()).unwrap_or("Column");
    ...
    let stats_str = if stats.numeric_count > 0 {
        ...
        format!("{}: count={} null={}", header, stats.count, stats.null_count)
    };
...
fn render_filter_popup(frame: &mut Frame, app: &App, area: Rect) {
    let (_filter_active, filter_mode, filter_col, filter_values, filter_selected, filter_cursor) = app.filter_info();
    ...
    let col_name = if let Some(col) = filter_col {
        app.data().headers().get(col).map(|h| h.as_str()).unwrap_or("Column")
    } else {
        "Column"
    };
    ...
    for (i, value) in filter_values.iter().enumerate().skip(start).take(max_visible) {
        if i >= end {
            break;
        }
    ...
    lines.push("─".repeat(area.width as usize - 2));
    lines.push(format!(" Selected: {}/{} | SPACE=toggle | Enter=apply | Esc=cancel", selected_count, total_count));
...
fn render_palette(frame: &mut Frame, area: Rect, query: String, cursor: usize, commands: Vec<String>) {
    ...
    let help_text = format!(" ↑/k ↓/j: Navigate | Enter: Execute | Esc: Close | Type: Filter {}", if query.is_empty() { "type to filter" } else { &query });
